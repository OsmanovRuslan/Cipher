...
def doing(request):
    if request.method == 'POST':
        ...
        key = generate_key_matrix(int(length))
        encoded_message = encode(message, key)
        decoded_message = "Ошибка. Повторите запрос"
        try:
        ...
        data = {}
    return render(request, 'cipher.html', data)
...
def inverse_matrix_mod_256(matrix):
    det = determinant(matrix)
    if det == 0:
        raise ValueError("Matrix is not invertible")
    det_inv = pow(det, -1, 256)
    adj = np.round(det_inv * np.linalg.inv(matrix) * det) % 256
...
def decode(message, key):
    ...
    for block in blocks:
        block_matrix = np.array([ord(char) for char in block])
        decoded_block_matrix = np.matmul(block_matrix, inverse_matrix_mod_256(key_matrix)) % 256
